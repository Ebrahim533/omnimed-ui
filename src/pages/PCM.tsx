...
const PCMPage = () => {
  const { servicePage, loading, error } = useServicePage("pcm");

  // Debug logging
  console.log("[PCM Debug] loading:", loading);
  console.log("[PCM Debug] error:", error);
  console.log("[PCM Debug] servicePage:", servicePage);
  ...
  if (error || !servicePage) {
    console.warn("[PCM Debug] Failed to load PCM from CMS, using fallback content. Error:", error, "ServicePage:", servicePage);
  ...
  console.log("[PCM Debug] heroImageUrl:", heroImageUrl);
